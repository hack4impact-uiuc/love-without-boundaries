        ...
        for (let i = 0; i < answers.length; i += 1) {
            if (answers[i].isCorrect) {
                ret = (
                    <p
                        key={idx}
                        style={{ color: '#22a531', paddingLeft: '20px' }}
                    >
        ...
        if (ret === null) {
            ret = (
                <p key={idx} style={{ color: '#22a531' }}>
                    {idx + 1}. There isnt a correct answer for this question.{' '}
                </p>
            );
        }
        return ret;
            ...
            <QueryRenderer
                ...
                render={({ props }) => {
                    ...
                    return (
                        <div>
                            <h1>
                                Past Quiz Submissions for Lesson:{' '}
                                <span style={{ color: '#2771aa' }}>
                                    {props.node.name}
                                </span>
                            </h1>
                            ...
                            <div className="row">
                                <div className="col-sm-5">
                                    {this.props.pastQuizzes
                                        ? this.props.pastQuizzes.map(
                                              (pq, idx) =>
                                                  pq.lessonID ===
                                                  this.props.lessonID ? (
                                                      <div
                                                          key={idx}
                                                          className="past-quiz-box"
                                                      >
                                                          <h4
                                                              style={{
                                                                  fontWeight:
                                                          ...
                                                          {pq.submittedAnswers.map(
                                                              (q, i) =>
                                                                  props.node.quiz.questions.some(
                                                                      e =>
                                                                          e.id ===
                                                                          q.questionID,
                                                                  ) ? (
